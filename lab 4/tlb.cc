...
void UpdateTLB(int possible_badVAddr)
{
  ...
  if((phyPage=VpnToPhyPage(vpn))!=-1) {
    InsertToTLB(vpn, phyPage); //vpn can be found in IPT
  } else {
    if(vpn>=currentThread->space->numPages && !GetMmap(vpn))
      machine->RaiseException(AddressErrorException, badVAddr);
    else //VPN cannot be found in IPT -> perform paging and update TLB
...
//if no empty entires
if (i == TLBSize) i=FIFO_Pointer;
 FIFO_Pointer = (i+1)%(TLBSize);
...
//return not valid page
  if(!memoryTable[phyPage].valid) return phyPage;
//check older page
  if((memoryTable[oldPage].lastUsed > memoryTable[phyPage].lastUsed)) oldPage = phyPage;
 }
//return oldest page 
 phyPage = oldPage; 
  return phyPage;
